  ...
  async generateUserPDF(req, res) {
    try {
      ...
      const createSection = (title) => {
        doc.rect(40, doc.y, doc.page.width - 80, 1).fill(colors.primary);
        doc.moveDown(0.3);
        doc.fontSize(10).font('Helvetica-Bold').fillColor(colors.primary)
           .text(title.toUpperCase(), 40, doc.y, { align: 'center', width: doc.page.width - 80 });
      ...
      const addField = (label, value, options = {}) => {
        ...
        doc.fontSize(8).font('Helvetica-Bold').fillColor(colors.textLight)
           .text(label, x, doc.y, { continued: true, width: 130 });
        doc.font('Helvetica').fillColor(colors.text)
           .text(value || 'N/A', { width: doc.page.width - 90 - indent - 130 });
        doc.moveDown(0.15);
      };

      // Función para subsección
      const addSubsection = (title) => {
        doc.fontSize(9).font('Helvetica-Bold').fillColor(colors.primary).text(title, 50);
      ...
      doc.fontSize(18).font('Helvetica-Bold').fillColor('#FFFFFF')
         .text('HOJA DE VIDA - PESV', 0, 12, { align: 'center' });
      doc.fontSize(9).fillColor('#FFFFFF').font('Helvetica')
         .text('Plan Estratégico de Seguridad Vial', 0, 33, { align: 'center' });
      
      doc.y = 65;
      doc.fontSize(7).fillColor(colors.textLight)
         .text(`Generado: ${new Date().toLocaleDateString('es-CO')}`, 50, doc.y);
      doc.text(`ID: ${cedula}`, doc.page.width - 150, 65, { width: 100, align: 'right' });
      ...
      if (surveyData) {
        createSection('Información Adicional');
        
        if (surveyData.consentimiento) {
          addField('Consentimiento:', surveyData.consentimiento);
        }

        // UBICACIÓN Y CARGO
        if (surveyData.ciudad || surveyData.sitioLabor || surveyData.cargo) {
          doc.moveDown(0.5);
          addSubsection('Ubicación y Cargo');
          if (surveyData.ciudad) addField('Ciudad:', surveyData.ciudad, { indent: 10 });
          if (surveyData.sitioLabor) addField('Sitio de labor:', surveyData.sitioLabor, { indent: 10 });
          if (surveyData.cargo) addField('Cargo:', surveyData.cargo, { indent: 10 });
        }
        ...
        if (surveyData.edad || surveyData.genero || surveyData.tipoContratacion || surveyData.grupo) {
          ...
          if (surveyData.edad) addField('Edad:', `${surveyData.edad} años`, { indent: 10 });
          if (surveyData.genero) addField('Género:', surveyData.genero, { indent: 10 });
          if (surveyData.tipoContratacion) addField('Tipo de contratación:', surveyData.tipoContratacion, { indent: 10 });
          if (surveyData.grupo) {
        ...
        if (surveyData.medioTransporteDesplazamiento || surveyData.claseVehiculo) {
          doc.moveDown(0.5);
          addSubsection('Transporte');
          if (surveyData.medioTransporteDesplazamiento) {
            addField('Medio de transporte:', surveyData.medioTransporteDesplazamiento, { indent: 10 });
          }
          if (surveyData.claseVehiculo) {
            const claseTexto = surveyData.claseVehiculo === 'Otro' && surveyData.claseVehiculoOtro
              ? `${surveyData.claseVehiculo} (${surveyData.claseVehiculoOtro})` : surveyData.claseVehiculo;
            addField('Clase de vehículo:', claseTexto, { indent: 10 });
          }
        }
        ...
        if (surveyData.licencia) {
          doc.moveDown(0.5);
          addSubsection('Licencia de Conducción');
          if (surveyData.licencia === 'SI') {
            if (surveyData.categoriaLicencia) addField('Categoría:', surveyData.categoriaLicencia, { indent: 10 });
            if (surveyData.experiencia) addField('Experiencia:', `${surveyData.experiencia} años`, { indent: 10 });
            if (surveyData.vigenciaLicencia) {
              ...
              addField('Vigencia:', vigencia + vencida, { indent: 10 });
            }
          } else {
            doc.fontSize(8).fillColor(colors.textLight).text('No posee', 60);
            doc.fillColor(colors.text);
          }
        }
        // DESPLAZAMIENTOS LABORALES
        if (surveyData.medioDesplazamiento?.length > 0 || surveyData.viasPublicas || 
            surveyData.frecuenciaVehiculoPropio || surveyData.usaVehiculoEmpresa) {
          doc.moveDown(0.5);
          addSubsection('Desplazamientos Laborales');
          
          if (surveyData.medioDesplazamiento?.length > 0) {
            addField('Medios de transporte:', surveyData.medioDesplazamiento.join(', '), { indent: 10 });
          }
          
          if (surveyData.viasPublicas) {
            addField('Usa vías públicas:', surveyData.viasPublicas, { indent: 10 });
          }
          
          if (surveyData.frecuenciaVehiculoPropio) {
            addField('Frecuencia vehículo propio:', surveyData.frecuenciaVehiculoPropio, { indent: 10 });
            if (surveyData.tipoVehiculoPropio) {
              const tipoTexto = surveyData.tipoVehiculoPropio === 'Otro' && surveyData.tipoVehiculoPropioOtro
                ? `${surveyData.tipoVehiculoPropio} (${surveyData.tipoVehiculoPropioOtro})` 
                : surveyData.tipoVehiculoPropio;
              addField('Tipo vehículo propio:', tipoTexto, { indent: 10 });
            }
            if (surveyData.empresaPagaRodamiento) {
              addField('Empresa paga rodamiento:', surveyData.empresaPagaRodamiento, { indent: 10 });
            }
            if (surveyData.realizaInspeccionPropio) {
              addField('Realiza inspección:', surveyData.realizaInspeccionPropio, { indent: 10 });
            }
            if (surveyData.frecuenciaChequeoPropio) {
              addField('Frecuencia chequeo:', surveyData.frecuenciaChequeoPropio, { indent: 10 });
            }
          }
          
          if (surveyData.usaVehiculoEmpresa) {
            addField('Usa vehículo empresa:', surveyData.usaVehiculoEmpresa, { indent: 10 });
            if (surveyData.usaVehiculoEmpresa === 'SI') {
              if (surveyData.tipoVehiculoEmpresa) {
                const tipoTexto = surveyData.tipoVehiculoEmpresa === 'Otro' && surveyData.tipoVehiculoEmpresaOtro
                  ? `${surveyData.tipoVehiculoEmpresa} (${surveyData.tipoVehiculoEmpresaOtro})` 
                  : surveyData.tipoVehiculoEmpresa;
                addField('Tipo vehículo empresa:', tipoTexto, { indent: 10 });
              }
              if (surveyData.realizaInspeccionEmpresa) {
                addField('Realiza inspección:', surveyData.realizaInspeccionEmpresa, { indent: 10 });
              }
              if (surveyData.frecuenciaChequeoEmpresa) {
                addField('Frecuencia chequeo:', surveyData.frecuenciaChequeoEmpresa, { indent: 10 });
              }
            }
          }
        }
        ...
        if (surveyData.accidente5Anios) {
          doc.moveDown(0.5);
          addSubsection('Accidentes (Últimos 5 años)');
          if (surveyData.accidente5Anios === 'SI') {
            if (surveyData.cantidadAccidentes) addField('Cantidad:', surveyData.cantidadAccidentes, { indent: 10 });
            if (surveyData.accidenteLaboral === 'SI' && surveyData.cantidadAccidentesLaborales) {
              addField('Laborales:', surveyData.cantidadAccidentesLaborales, { indent: 10 });
            }
            if (surveyData.rolAccidente) addField('Rol:', surveyData.rolAccidente, { indent: 10 });
          } else {
            doc.fontSize(8).fillColor(colors.textLight).text('Sin accidentes', 60);
            doc.fillColor(colors.text);
          }
        }
        ...
        if (surveyData.tieneComparendos) {
          doc.moveDown(0.5);
          addSubsection('Comparendos');
          if (surveyData.tieneComparendos === 'SI' && surveyData.causasComparendo && surveyData.causasComparendo.length > 0) {
            doc.fontSize(8).font('Helvetica').fillColor(colors.text);
            surveyData.causasComparendo.forEach(causa => {
              doc.text(`• ${causa}`, 60);
            });
            if (surveyData.causaComparendoOtra) {
              doc.text(`• Otra: ${surveyData.causaComparendoOtra}`, 60);
            }
          } else {
            doc.fontSize(8).fillColor(colors.textLight).text('Sin comparendos', 60);
            doc.fillColor(colors.text);
          }
        }
        ...
        if (surveyData.planificacion || surveyData.antelacion || surveyData.kmMensuales) {
          doc.moveDown(0.5);
          addSubsection('Planificación de Desplazamientos');
          if (surveyData.planificacion) addField('Planifica:', surveyData.planificacion, { indent: 10 });
          if (surveyData.antelacion) addField('Antelación:', surveyData.antelacion, { indent: 10 });
          if (surveyData.kmMensuales) addField('Km mensuales:', `${surveyData.kmMensuales} km`, { indent: 10 });
        }

        // Información adicional
        if (surveyData.informacionAdicional) {
          doc.moveDown(0.5);
          addSubsection('Observaciones');
          doc.fontSize(7).font('Helvetica').fillColor(colors.text)
             .text(surveyData.informacionAdicional, 60, doc.y, { 
        ...
        if (surveyData.fechaRegistro) {
          ...
          doc.fontSize(7).fillColor(colors.textLight).font('Helvetica-Oblique')
             .text(`Registrado: ${new Date(surveyData.fechaRegistro).toLocaleDateString('es-CO')}`, 50);
          doc.fillColor(colors.text);
        }
      } else {
        doc.fontSize(8).fillColor(colors.textLight).font('Helvetica-Oblique')
           .text('El usuario aún no ha completado la información adicional.', 50);
        doc.fillColor(colors.text);
      }

      // FOOTER SIMPLE (sin crear páginas adicionales)
      doc.moveDown(1.5);
      doc.strokeColor(colors.border).lineWidth(0.5)
         .moveTo(40, doc.y).lineTo(doc.page.width - 40, doc.y).stroke();
      doc.moveDown(0.3);
      doc.fontSize(7).fillColor(colors.textLight).font('Helvetica')
         .text('Control Vehicular - Sistema de Gestión de Seguridad Vial', { align: 'center' });
